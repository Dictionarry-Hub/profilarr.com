...
import { qualityProfileToMarkdown } from '$lib/shared/utils/llm/index.js';
...
export const GET: RequestHandler = ({ params }) => {
	...
	const data = entry[1];
	const profile = data.qualityProfiles.find((qp) => slugify(qp.name) === params.slug);
	...
	return new Response(
		qualityProfileToMarkdown(
			data,
			profile,
			entityHistory(data, 'quality_profile', profile.name)
		),
		{
