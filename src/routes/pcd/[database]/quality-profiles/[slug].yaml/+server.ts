...
import { qualityProfileToYaml } from '$lib/shared/utils/yaml/index.js';
...
export const GET: RequestHandler = ({ params }) => {
	...
	const data = entry[1];
	const profile = data.qualityProfiles.find((item) => slugify(item.name) === params.slug);
	if (!profile) error(404, 'Quality profile not found');

	return new Response(qualityProfileToYaml(data, profile), {
		headers: {
