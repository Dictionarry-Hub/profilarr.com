...
Elo ranking is controlled at build time by `PUBLIC_SEARCH_ELO_ENABLED`. It defaults to `false`. When
disabled, query results use text scores only, the popular empty state is hidden, and per-term
